...
def crt_factory(INT_TYPE, MODULI):  # pylint: disable=invalid-name
    ...
    def gen_crt_recombine_explicit():
        ...
        def crt_recombine_explicit(x, bound):
            ...
            big_b = MODULUS % bound
            b = [
                np.array([(MODULUS // mi) % bound], dtype=np.int64)
                for mi in MODULI
            ]
            ...
            with tf.name_scope('crt_recombine_explicit'):
                ...
                if isinstance(x[0], tf.Tensor):
                    ...
                    alpha = tf.round(
                        tf.reduce_sum(
                            [
                                tf.cast(ti, tf.float32) / mi for ti,
                                mi in zip(t, MODULI)
                            ],
                            axis=0
                        )
                    ...
                    u = tf.cast(
                        tf.reduce_sum(
                            [ti * bi for ti, bi in zip(t, b)], axis=0
                        ),
                        tf.int64
                    )
                ...
                raise TypeError(
                    "Don't know how to recombine {}".format(type(x[0]))
                )
    ...
    class Factory(AbstractFactory):
        ...
        def sample_bounded(self, shape, bitlength: int):
            ...
            if secure_random.supports_seeded_randomness():
                ...
                return BoundedTensor(
                    shape=shape, seeds=seeds, chunk_sizes=chunk_sizes
                )
            ...
            chunk_values = [
                sampler(
                    shape=shape, minval=0, maxval=2**chunk_size, dtype=INT_TYPE
        ...
        def tensor(self, value):
            ...
            raise TypeError(
                ("Don't know how to handle ", "{}".format(type(value)))
            )
    ...
    class Tensor(AbstractTensor):
        ...
        def bits(
            ...
            with tf.name_scope('to_bits'):
                ...
                chunk_bitsizes = [max_chunk_bitsize
                                 ] * q + ([r] if r > 0 else [])
                ...
                chunks_bits = [
                    binarize(chunk, chunk_bitsize) for chunk,
                    chunk_bitsize in zip(chunks, chunk_bitsizes)
                ]
        ...
        def cumsum(self, axis, exclusive, reverse):
            with tf.name_scope('crt_cumsum'):
                backing = [
                    tf.cumsum(
                        xi, axis=axis, exclusive=exclusive, reverse=reverse
                    ) % mi for (xi, mi) in zip(self.backing, MODULI)
                ]
                return DenseTensor(backing)
        ...
        def equal_zero(self, factory=None):
            ...
            with tf.name_scope('crt_equal_zero'):
                zeros = [
                    tf.cast(tf.equal(xi, 0), factory.native_type)
        ...
        def im2col(
            self, h_filter: int, w_filter: int, padding: str, stride: int
        ):
            ...
            with tf.name_scope("crt_batch_to_space_nd"):
                backing = [
                    tf.batch_to_space_nd(
                        xi, block_shape=block_shape, crops=crops
                    ) for xi in self.backing
                ]
                return DenseTensor(backing)
            ...
            with tf.name_scope("crt_space_to_batch_nd"):
                backing = [
                    tf.space_to_batch_nd(
                        xi, block_shape=block_shape, paddings=paddings
                    ) for xi in self.backing
                ]
                return DenseTensor(backing)
        ...
        def strided_slice(self, args, kwargs):
            backing = [
                tf.strided_slice(xi, *args, **kwargs) for xi in self.backing
            ]
            return DenseTensor(backing)
        ...
        def split(self, num_split: Union[int, list], axis: int = 0):
            backings = zip(
                *[tf.split(xi, num_split, axis=axis) for xi in self.backing]
            )
        ...
        def negative(self):
            backing = [
                tf.negative(xi) % mi for xi, mi in zip(self.backing, MODULI)
            ]
            return DenseTensor(backing)
    ...
    class BoundedTensor(Tensor):
        ...
        def backing(self):
            with tf.name_scope('expand-seed'):
                ...
                return _construct_backing_from_chunks(
                    self._chunk_sizes, chunk_values
                )
    ...
    class Constant(DenseTensor, AbstractConstant):
        ...
        def __init__(self, backing) -> None:
            assert all(
                isinstance(component, tf.Tensor) for component in backing
            )
    ...
    class Placeholder(DenseTensor, AbstractPlaceholder):
        ...
        def __init__(self, shape) -> None:
            self.placeholders = [
                tf.placeholder(INT_TYPE, shape=shape) for _ in MODULI
            ]
    ...
    class Variable(DenseTensor, AbstractVariable):
        ...
        def __init__(self, initial_backing) -> None:
            ...
            self.initializer = tf.group(
                *[var.initializer for var in self.variables]
            )
