...
def patches2im(patches, patch_size, stride=1, padding='SAME', img_size=None, consolidation="SUM", data_format="NCHW"):
    """
    Reconstructed image fromt extracted patches.
    See: https://stackoverflow.com/a/56891806

    :param patches: a 4-D Tensor in 'NCHW' or "NHWC" format where the depth dimension ('C') represents a patch
    :param img_size: a tuple (height, width) representing output image size (optional). If `None`, image size
        will be inferred from inputs.
    """
    with tf.variable_scope('patches2im'):
        ...
        wout = (_w - 1) * stride + ps_w  # Recalculate output shape of "extract_image_patches" including padded pixels
        hout = (_h - 1) * stride + ps_h  # Recalculate output shape of "extract_image_patches" including padded pixels

        x, y = tf.meshgrid(tf.range(ps_w), tf.range(ps_h))
        x = tf.reshape(x, (1, 1, ps_h, ps_w, 1, 1))
        y = tf.reshape(y, (1, 1, ps_h, ps_w, 1, 1))
        xstart, ystart = tf.meshgrid(tf.range(0, (wout - ps_w) + 1, stride),
        ...
        with tf.variable_scope("consolidate"):
            ...
            if consolidation == "SUM":
                ...
                raise NotImplementedError("Unknown consolidation method: {}".format(consolidation))

        if img_size is not None:
            img_h, img_w = img_size
            if img_h > hout:
                # This happens when padding is 'VALID' and the image has been cropped, hence we will just pad 0 at the end
                reconstructed_img = tf.pad(reconstructed_img, [[0, 0], [0, img_h - hout], [0, 0], [0, 0]])
            elif img_h < hout:
                pad_top = (hout - img_h) // 2
                pad_bottom = hout - img_h - pad_top
                reconstructed_img = reconstructed_img[:, pad_top:(hout-pad_bottom), :, :]

            if img_w > wout:
                reconstructed_img = tf.pad(reconstructed_img, [[0, 0], [0, 0], [0, img_w - hout], [0, 0]])
            elif img_w < wout:
                pad_left = (wout - img_w) // 2
                pad_right = wout - img_w - pad_left
                reconstructed_img = reconstructed_img[:, :, pad_left:(wout-pad_right), :]
...
def im2col(
    ...
    with tf.name_scope("im2col"):
        ...
        patch_tensor = im2patches(x, (h_filter, w_filter), stride=stride, padding=padding, data_format="NHWC")
...
# def col2im(x, h_filter, w_filter, stride=1, padding='SAME', img_size=None, consolidation="SUM"):
    # assert x.shape[0] % (h_filter * w_filter) == 0, "Invalid dimension: {} % ({} x {}) != 0".formmat(x.shape[0], h_filter, w_filter)
    # channels = x.shape[0] // (h_filter * w_filter)
    # x = tf.reshape(x, [channels, h_filter, w_filter, -1])
    # x = tf.transpose(x, [1, 2, 0, 3])

    # x = tf.reshape(x, [h_filter * w_filter * channels, ])
...
def conv2d(x, y, stride=1, padding="SAME", data_format="NCHW"):
    ...
    with tf.name_scope("conv2d"):
        ...
        h_out, w_out = out_size([h_x, w_x], [h_filter, w_filter], [stride, stride], padding)

        x_col = im2col(x, h_filter, w_filter, stride=stride, padding=padding, data_format="NHWC")
