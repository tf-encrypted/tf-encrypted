...
def _fp_div(prot: "prot", a: "PrivateTensor", b: "PrivateTensor", nonsigned: bool):
    with tf.name_scope("fp_div"):
        return a * _fp_recip_private(prot, b, nonsigned)

def _fp_div_private_private(prot: "prot", a: "PrivateTensor", b: "PrivateTensor", nonsigned: bool):
    return _fp_div(prot, a, b, nonsigned)

def _fp_div_public_private(prot: "prot", a: "PublicTensor", b: "PrivateTensor", nonsigned: bool):
    return _fp_div(prot, a, b, nonsigned)

def _fp_div_private_public(prot: "prot", a: "PrivateTensor", b: "PublicTensor", nonsigned: bool):
    return a / b

def _fp_sqrt2(prot: "prot", a: "PrivateTensor"):
...
def _fp_recip_private(prot, x: "PrivateTensor", nonsigned):
    """
        Approxiamtedly compute 1/x from x.

        Apply the quintic iteration from http://numbers.computation.free.fr/Constants/Algorithms/inverse.html
    """
    with tf.name_scope("fp_reciprocal"):
        sgf, exp = __fp_normalize(prot, x, nonsigned) # x = sgf / exp, then 1 / x = 1/sgf * exp
        one = prot.define_constant(1.)
        two = prot.define_constant(2, apply_scaling=False)
        ## 2.9281928 - 2 * s approxiates 1/s with small relative errors in the interval s \in [0.5, 1.)
        ...
        for _ in range(1):
            ...
            res2 = res * res
            appr_recip = appr_recip + appr_recip * (one + res2) * (res + res2) # quintic iteration
        return appr_recip

def _fp_inv_sqrt_private(prot: "prot", a: "PrivateTensor"):
    ## low precision
...
def _fp_sqrt_private(prot: "prot", a: "PrivateTensor"):
...
def _do_fp_log_private(prot: "prot", x: "PrivateTensor", base: "float"):
    k = prot.fixedpoint_config.precision_fractional
    m = k + prot.fixedpoint_config.precision_integral
    n = prot.int_factory.nbits
    logn = int(log2(n) + 1) * 2 # enough bit length to represent the exponent \in [0, 128)
    assert x.is_scaled, "Er.. tricky here."
    assert 2*k > m, "We assume 2^{-j} can be represent with 2k-bit precisions for all j in [0, m)"
    assert base >= 2., "log(x, base) shoule with base >= 2."
    adjust = 1. / np.log2(base)
    with tf.name_scope("fp_log"):
        ...
        log_exponent = prot.b2a(prot.xor_indices(z_bits), logn) * 2**k # j + k with k-bit precision
        ...
        ## The approximation coefficients are for log2(x), we need to adjust them via multiplying 1./log2(base)
        """ """
        log_frac = ((-0.4326728 * adjust) * frac + (2.276597 * adjust)) * frac + (-1.843924 * adjust)
        return log_frac + log_exponent

def _fp_log2_private(prot: "prot", x: "PrivateTensor"):
    return _do_fp_log_private(prot, x, 2.)

def _fp_log10_private(prot: "prot", x: "PrivateTensor"):
    return _do_fp_log_private(prot, x, 10.)

def _fp_ln_private(prot: "prot", x: "PrivateTensor"):
    return _do_fp_log_private(prot, x, np.e)

def __fp_normalize(prot, b: "PrivateTensor", nonsigned=False):
    """
        Given [b], to compute [sgf], and [exp] such that b = sgf / exp where sgf \in [0.5, 1)
    """
    k = prot.fixedpoint_config.precision_fractional
    m = k + prot.fixedpoint_config.precision_integral
    ...
    with tf.name_scope("fp_normalize"):
        ...
        """
            ...
            Also, if we right shift {z'_i} by (128 - m)-steps. Then we obtain 2^{m - j},
            which equals to the fixed-point representation of 2^{-j} within m-bits precision.
        """
        ## NOTE: We couldn't obtain 2^{-j} with only k-bits precision because j > k might be
        ## possible (i.e., b = 2^k * b' for real value b' > 1)
...
def approx_sqrt_inv(prot, x: "PrivateTensor"):
    """
    ...
    with tf.name_scope("normalize_for_sqrt"):
        ...
        exponet = prot.b2a(prot.bit_gather(rev_z_bits | rev_z_bits >> 1, 0, 2), k) # 2^{floor(-(j+k)/2)}
        ...
        if k & 1 == 0: # k is even which means lsb = 1 <=> j is odd
            ...
            exponet = exponet * select(2**(k//2) * np.sqrt(2.), 2**(k//2 + 1), lsb)
