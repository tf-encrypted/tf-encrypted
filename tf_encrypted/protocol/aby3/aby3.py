...
class ABY3(Protocol):
    ...
    def __init__(
        ...
        self.b2a_keys_1, self.b2a_keys_2, self.b2a_nonce = \
            self.setup_b2a_generator()
    ...
    """
        ...
        assert tensor.shape.is_fully_defined(), (
            "Shape of input '{}' is not fully defined".
    ...
    def gather(self, x, indices, axis):
        raise NotImplementedError(
            "Unsupported share type: {}".format(x.share_type)
        )
    ...
    def iterate(
        ...
        return self.dispatch(
            "iterate", tensor, batch_size, repeat, shuffle, seed
        )
...
class ABY3Constant(ABY3PublicTensor):
  ...
  """
    ...
    def __init__(self, prot, constants, is_scaled, share_type):
        ...
        super(ABY3Constant,
              self).__init__(prot, constants, is_scaled, share_type)
...
def _negative_public(prot, x):
    ...
    with tf.name_scope("negative"):
        ...
        x_neg = ABY3PublicTensor(
            prot, [x_on_0_neg, x_on_1_neg, x_on_2_neg],
            x.is_scaled,
            x.share_type
        )
    return x_neg
    ...
    with tf.name_scope("bit_extract"):
        if x.share_type == ARITHMETIC:
            with tf.name_scope("A2B_partial"):
                ...
                operand1 = ABY3PrivateTensor(
                    prot, operand1, x.is_scaled, BOOLEAN
                )
                operand2 = ABY3PrivateTensor(
                    prot, operand2, x.is_scaled, BOOLEAN
                )
  ...
  """
    ...
    with tf.name_scope("B2A"):
        ...
        neg_x1_neg_x2 = ABY3PrivateTensor(
            prot, neg_x1_neg_x2, x.is_scaled, BOOLEAN
        )
...
def _polynomial_private(prot, x, coeffs):
    ...
    with tf.name_scope("polynomial"):
        result = prot.define_constant(
            np.zeros(x.shape), apply_scaling=x.is_scaled
        )
        for i in range(len(coeffs)):
            if i == 0:
                ...
                tmp = prot.define_constant(
                    np.array([coeffs[i]]), apply_scaling=False
                )
                tmp = tmp * (x**i)
                result = result + tmp
            else:
    ...
    def helper(idx):
        with tf.device(prot.servers[idx].device_name):
            ...
            for i in range(2):
                ...
                iterators[idx][
                    i] = tf.compat.v1.data.make_initializable_iterator(dataset)
