  ...
  """  # noqa:E501
    ...
    """
        ...
        with tf.name_scope("public-var{}".format("-" + name if name else "")):
            ...
            if isinstance(initial_value, np.ndarray):
                ...
                v = self._encode(
                    initial_value, apply_scaling, tf_int_type=inttype
                )
                v_on_0, v_on_1 = v, v
    ...
    """
        ...
        def helper(v: tf.Tensor) -> "PondPublicTensor":
            ...
            enc = self._encode(
                v, apply_scaling, tf_int_type=factory.native_type
            )
        ...
        with tf.name_scope("public-input{}".format(suffix)):
            ...
            with tf.device(player.device_name):
                ...
                raise TypeError(
                    ("Don't know how to handle inputs "
        ...
        def decorator(compute_func):
            ...
            def compute_func_wrapper(*compute_func_args, **compute_func_kwargs):
                ...
                if actual_player_name is None:
                    ...
                    actual_player_name = compute_func_kwargs.get(
                        "player_name", None
                    )
                if actual_player_name is None:
    ...
    """  # noqa:E501
        ...
        def share_output(v: tf.Tensor):
            ...
            return PondMaskedTensor(
                self, x, a, a0, a1, alpha, alpha, apply_scaling
            )
        ...
        with tf.name_scope("encode"):
            ...
            if isinstance(scaled, np.ndarray):
                ...
                tf_int_type = tf_int_type or (
                    scaled.dtype if scaled.dtype in TF_INT_TYPES else
                    self.tensor_factory.native_type
                )
    ...
    def assign(
        ...
        if isinstance(variable, PondPrivateVariable):
            ...
            raise TypeError(
                ("Don't know how to handle variable "
    ...
    """
        ...
        if isinstance(y, int):
            ...
            raise TypeError(
                "Don't know how to divide by type {}".format(type(y))
            )
    ...
    """
        ...
        if isinstance(x, PondPublicTensor):
            ...
            raise TypeError(
                ("Don't know how to do a strided slice on "
                 " {}").format(type(x))
            )
...
def _add_public_public(prot, x, y):
    ...
    assert x.is_scaled == y.is_scaled, (
        "Cannot mix different encodings: {} {}".format(
    ...
    with tf.name_scope("matmul"):
        ...
        with tf.device(prot.server_0.device_name):
            with tf.name_scope("combine"):
                alpha = alpha_on_0
                beta = beta_on_0
                z0 = ab0 + a0.matmul(beta) + alpha.matmul(b0
                                                         ) + alpha.matmul(beta)
...
def _indexer_public(
        prot: Pond, tensor: PondPublicTensor, slc
) -> "PondPublicTensor":
...
def _indexer_private(
        prot: Pond, tensor: PondPrivateTensor, slc
) -> "PondPrivateTensor":
...
def _indexer_masked(
        prot: Pond, tensor: PondMaskedTensor, slc
) -> "PondMaskedTensor":
...
def _split_public(
    ...
    with tf.name_scope("split"):
        ...
        return [
            PondPublicTensor(prot, y_on_0, y_on_1, x.is_scaled) for y_on_0,
            y_on_1 in zip(ys_on_0, ys_on_1)
        ]
...
def _split_private(
    ...
    with tf.name_scope("split"):
        ...
        return [
            PondPrivateTensor(prot, y0, y1, x.is_scaled) for y0,
            y1 in zip(ys0, ys1)
        ]
...
def _split_masked(
    ...
    with tf.name_scope("split"):
        ...
        return [
            PondMaskedTensor(
                prot, y, b, b0, b1, beta_on_0, beta_on_1, x.is_scaled
