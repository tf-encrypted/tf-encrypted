...
SEED = [87654321, 87654321, 87654321, 87654321,
...
class TestSeededRandom(unittest.TestCase):
  ...
  def test_invalid_args(self):
    with tf.Session():
      ...
      with np.testing.assert_raises(ValueError):
        secure_random.seeded_random_uniform(
            [3, 3], seed=SEED, maxval=10000, dtype=tf.float32).eval()
    ...
    with tf.Session():
      ...
      with np.testing.assert_raises(ValueError):
        secure_random.random_uniform(
            [3, 3], maxval=10000, dtype=tf.float32).eval()
...
class TestSeed(unittest.TestCase):
  ...
  def test_seed_generation(self):
    with tf.Session():
      ...
      output = secure_random.seeded_random_uniform(
          shape, seed=s, minval=minval, maxval=maxval).eval()
