...
kill -9 `ps -ef | grep ${ABS_DIRECTORY}/private_lr_training.py | grep -v grep | awk '{print $2}'`
...
(taskset -c 12,13,14,15 python -m tf_encrypted.player training-client --config ${ABS_DIRECTORY}/config.json > log3.txt 2>&1 &)
sleep 1
...
(time taskset -c 20,21,22,23 python ${ABS_DIRECTORY}/private_lr_training.py ${ABS_DIRECTORY}/config.json > log.txt 2>&1 &)
