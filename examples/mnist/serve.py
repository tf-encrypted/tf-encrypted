...
class PredictionClient(tfe.io.InputProvider, tfe.io.OutputReceiver):
    ...
    def provide_input(self) -> tf.Tensor:
        prediction_input_queue = tf.FIFOQueue(capacity=10, dtypes=[tf.float32], shapes=[1, 28 * 28], shared_name='prediction_input_queue')
        prediction_input = prediction_input_queue.dequeue()
        return prediction_input

    def receive_output(self, likelihoods: tf.Tensor) -> tf.Operation:
        prediction = tf.argmax(likelihoods, axis=1)
        op = tf.Print([], [prediction], summarize=10, message="ACTUAL ")
        return op
...
with config.session(target) as sess:
    ...
    print("Enqueue prediction")
    prediction_input_queue = tf.FIFOQueue(capacity=10, dtypes=[tf.float32], shared_name='prediction_input_queue')
    prediction_input = tf.placeholder(dtype=tf.float32)
    enqueue_op = prediction_input_queue.enqueue(prediction_input)
    sess.run(enqueue_op, feed_dict={prediction_input: np.zeros([28*28])})
...
#     prediction_input_queue = tf.FIFOQueue(10, [28 * 28], shared_name='prediction_input_queue')
#     prediction_input = tf.placeholder(dtype=tf.float32)
#     enqueue_op = prediction_input_queue.enqueue(prediction_input)
