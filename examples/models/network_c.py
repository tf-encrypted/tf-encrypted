...
def network_c(batch_input_shape, classes, private=True):
    if private:
        ...
        model.add(tfe.keras.layers.Dense(500, activation=None, lazy_normalization=True))
        model.add(tfe.keras.layers.ReLU())
        model.add(
        ...
        model.add(tf.keras.layers.Conv2D(50, 5, 1, padding="valid", activation=None))
        model.add(tf.keras.layers.MaxPooling2D(2))
        model.add(tf.keras.layers.ReLU())
