...
class Dense(Layer):
    ...
    def initialize(self):
        initial_weights = np.random.normal(
            scale=0.1, size=(self.num_inputs, self.num_nodes))
        self.weights = self.prot.define_private_variable(initial_weights)
        self.bias = self.prot.define_private_variable(
            np.zeros(1, self.num_nodes))
...
class Conv2D(Layer):
    def __init__(
        self, filter_shape, strides=1, padding="SAME",
    ...
    def initialize(self, input_shape, initial_weights=None):
        ...
        strides = float(self.strides)
        if self.padding == "SAME":
            h_out = int(math.ceil(float(h_x) / strides))
            w_out = int(math.ceil(float(w_x) / strides))
        if self.padding == "VALID":
            h_out = int(math.ceil(float(h_x - h_filter + 1) / strides))
            w_out = int(math.ceil(float(w_x - w_filter + 1) / strides))

        if initial_weights is None:
            initial_weights = self.filter_init(self.fshape)
        self.weights = self.prot.define_private_variable(initial_weights)
        self.bias = self.prot.define_private_variable(
            np.zeros((n_filters, h_out, w_out)))
    ...
    def backward(self, d_y, learning_rate):
        ...
        if self.model.layers.index(self) != 0:
            ...
            dx = W_reshaped.dot(dout_reshaped).col2im(
                imshape=self.cached_input_shape, field_height=h_filter,
